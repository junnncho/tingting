...
export const Select = ({
  ...
  const isOpenClassName = isOpen ? "ring-2 ring-color-main-100 border-color-main" : "";
  return (
    ...
    >
      <div className={twMerge("relative inline-block min-w-[100px] ", className)} style={{ ...style }} ref={selectRef}>
        <div
          className={twMerge(
            `w-full flex justify-between items-center gap-2 px-2 py-1 border rounded-md cursor-pointer duration-400 transition-all`,
            isOpenClassName,
            disabledClassName,
            selectClassName
          )}
        ...
        >
          {(!selectedValues.length || selectedValues?.[0] === "") && placeholder && (
              ...
              <div className={`min-w-[10px] min-h-[18px] duration-400 transition-all ${isOpen && "opacity-40"}`}>
                <div className="flex flex-wrap gap-1">
                  {selectedValues
                    .filter((value) => value !== "")
                    .map((value) => (
                      <div className="inline-block px-1 bg-gray-100 border rounded-md">
        ...
        <animated.div
          className="scrollbar-thin scrollbar-thumb-color-main-200 scrollbar-track-color-main-50 absolute z-50 w-full max-h-[240px] overflow-y-auto p-1 origin-top bg-white rounded-md drop-shadow-lg"
  ...
  return (
    <div
      className={`flex justify-between cursor-pointer px-2 py-1 rounded-md truncate hover:bg-gray-100 duration-400 transition-all ${
        selectedValues.some((cur) => cur === value) ? "bg-color-main-100 hover:bg-color-main-100 font-bold" : ""
      }`}
      onClick={() => handleOptionClick(value)}
    >
      <div>{children}</div>
      {isMultiple && selectedValues.some((cur) => cur === value) && <AiOutlineCheck className="text-color-main" />}
    </div>
  );
};
