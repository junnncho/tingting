...
import { DatePicker, Input, Select } from "@shared/ui-web";
import { Form, Upload, UploadFile } from "antd";
import { InitActionForm, Slice } from "@shared/util-client";
import { ReactNode, useEffect, useRef, useState } from "react";
import { Utils, locationMap } from "@shared/util";
    ...
    <div>
      <FieldWrapper className="flex-wrap">
        ...
        <Upload
          ...
          onRemove={(file) => onUpdate(files?.filter((f) => f.id !== file.uid) ?? [])}
        >
          <div>
            <AiOutlinePlus />
            <div style={{ marginTop: 8 }}>Upload</div>
...
export const Password = ({ label, value, onChange }: PasswordProps) => {
  return <Input.Password value={value ?? ""} onChange={(e) => onChange(e.target.value)} />;
};
...
export const Parent = <T extends string, State, M extends { id: string }, L extends { id: string }>({
  ...
  useEffect(() => {
    !value && slice.do[`init${Utils.capitalize(slice.refName)}`](init);
  }, [value]);
  return (
