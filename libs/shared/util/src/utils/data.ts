...
export const getAge = (birth: Dayjs) => {
  return new Date().getFullYear() - birth.year() + 1;
};
...
export const deepObjectify = (obj: any): any => {
  if (isDayjs(obj) || obj?.constructor === Date) return obj;
  else if (Array.isArray(obj)) {
    return obj.map(deepObjectify);
  } else if (obj && typeof obj === "object") {
    const val: any = {};
    Object.keys(obj).forEach((key) => {
      if (!obj.__ModelType__) val[key] = obj[key];
      else if (typeof obj[key] !== "function") val[key] = deepObjectify(obj[key]);
    });
    return val;
  } else {
    return obj;
