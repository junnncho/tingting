import { CurrencyInput, CurrencySchema } from "./currency.gql";
import { Document, Model, Query, Schema as Sch, Types } from "mongoose";
import { Schema, SchemaFactory } from "@nestjs/mongoose";
import { dbConfig } from "@shared/util-server";
@Schema(dbConfig.defaultSchemaOptions)
class Currency extends CurrencySchema {}
export const name = Currency.name;
export type Input = CurrencyInput;
export type Raw = Currency;
...
schema.index({ name: "text", symbol: "text", services: "text" });
