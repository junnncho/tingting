...
import { GroupCallInput, GroupCallSchema } from "./groupCall.gql";
import { Id, dbConfig } from "@shared/util-server";
import { Schema, SchemaFactory } from "@nestjs/mongoose";
@Schema(dbConfig.defaultSchemaOptions)
class GroupCall extends GroupCallSchema {}
export const name = GroupCall.name;
export type Input = GroupCallInput;
export type Raw = GroupCall;
...
interface DocMtds extends dbConfig.DefaultDocMtds<Doc> {
  dumb: () => Doc;
  addUser: (userId: Id) => Promise<Doc>;
}
schema.methods.dumb = function (this: Doc) {
  return this;
};
schema.methods.addUser = async function (this: Doc, userId: Id) {
  return await this.merge({ users: [...this.users, userId] }).save();
};
...
export const middleware = () => () => {
  ...
  schema.pre<Doc>("save", async function (next) {
    ...
    else if (this.status === "active" && this.isModified("status")) model.subSummary(["total", this.status]);
