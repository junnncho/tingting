...
import { BoardInput, BoardSchema } from "./board.gql";
import { Document, Model, Query, Schema as Sch, Types } from "mongoose";
import { Schema, SchemaFactory } from "@nestjs/mongoose";
import { dbConfig } from "@shared/util-server";
@Schema(dbConfig.defaultSchemaOptions)
class Board extends BoardSchema {}
export const name = Board.name;
export type Input = BoardInput;
export type Raw = Board;
...
interface DocMtds extends dbConfig.DefaultDocMtds<Doc> {
  isGranted: (creator?: db.User.Doc | null) => boolean;
}
schema.methods.isGranted = function (this: Doc, creator?: db.User.Doc) {
  if (this.roles.includes("guest")) return true;
  else if (!creator) return false;
  else if (this.roles.some((role) => creator.roles.includes(role as any))) return true;
