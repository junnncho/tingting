...
export interface DocMtds extends shared.User.DocMtds {
  addBoard: (chatBoardId: Id) => Doc;
  removeBoard: (chatBoardId: Id) => Doc;
}
schema.methods.addBoard = function (this: Doc, chatBoardId: Id) {
  for (const id of this.chatBoards) if (chatBoardId.equals(id)) return this;
  this.chatBoards.push(chatBoardId);
  return this;
};

schema.methods.removeBoard = function (this: Doc, chatBoardId: Id) {
  this.chatBoards = this.chatBoards.filter((id) => !chatBoardId.equals(id));
  return this;
};
