...
import { ReportInput, ReportSchema } from "./report.gql";
import { Schema, SchemaFactory } from "@nestjs/mongoose";
@Schema(dbConfig.defaultSchemaOptions)
class Report extends ReportSchema {}
export const name = Report.name;
export type Input = ReportInput;
export type Raw = Report;
...
interface DocMtds extends dbConfig.DefaultDocMtds<Doc> {
  process: (adminId: Id) => Doc;
  resolve: (adminId: Id, replyContent: string) => Doc;
}
schema.methods.process = function (this: Doc, adminId: Id) {
  if (this.status !== "active") throw new Error("Already Processed");
  (this.constructor as Mdl).moveSummary(this.status, "inProgress");
  this.merge({ status: "inProgress", replyFrom: adminId });
  return this;
};
schema.methods.resolve = function (this: Doc, adminId: Id, replyContent: string) {
  // if (!["active", "inProgress"].includes(this.status)) throw new Error("Already Resolved");
  (this.constructor as Mdl).moveSummary(this.status, "resolved");
  this.merge({
    status: "resolved",
    replyFrom: adminId,
    replyContent,
