import { CommentInput, CommentSchema } from "./comment.gql";
...
export const name = Comment.name;
export type Input = CommentInput;
export type Raw = Comment;
...
interface MdlStats extends dbConfig.DefaultMdlStats<Doc, Raw> {
  like: (commentId: Id, prev?: number) => Promise<boolean>;
  resetLike: (commentId: Id, prev: number) => Promise<boolean>;
  unlike: (commentId: Id, prev?: number) => Promise<boolean>;
}
