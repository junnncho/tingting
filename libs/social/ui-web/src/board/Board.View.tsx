...
export const BoardView = ({ className, board, slice = st.slice.board }: BoardViewProps) => {
  const { l } = usePage();
  return (
    <div className={twMerge(className, ``)}>
      ...
      <div className="flex justify-between p-4 mt-0 text-xs bg-gray-50 md:text-base">
        ...
        <RecentTime date={board.createdAt} breakUnit="second" timeOption={{ dateStyle: "short", timeStyle: "short" }} />
      </div>
    </div>
  );
};
