...
import { UserInput, UserSchema } from "./user.gql";
import { db as shared } from "@shared/module";
import { db as social } from "@social/module";

@Schema(dbConfig.defaultSchemaOptions)
class User extends UserSchema {}
// eslint-disable-next-line @typescript-eslint/no-empty-interface
interface User extends shared.User.Raw, social.User.Raw {}
export const name = User.name;
export type Input = UserInput & shared.User.Input & social.User.Input;
export type Raw = User & shared.User.Raw & social.User.Raw;
export interface DocType extends Document<Types.ObjectId, QryHelps, Raw>, DocMtds, Omit<Raw, "id"> {}
export type Doc = DocType & shared.User.DocType & social.User.DocType & dbConfig.DefaultSchemaFields;
export type Mdl = Model<Doc, QryHelps, DocMtds> & MdlStats & shared.User.Mdl & social.User.Mdl;
...
interface DocMtds extends shared.User.DocMtds, social.User.DocMtds {
  getPoint: (point: number) => Doc;
}
schema.methods.getPoint = function (this: Doc, point: number) {
  this.point += point;
  return this;
};

// * 5. 2. Model Statics
interface MdlStats extends shared.User.MdlStats, social.User.MdlStats {
  dumb: () => Promise<Doc>;
}
...
// * 5. 3. Model Statics
interface QryHelps extends shared.User.QryHelps, social.User.QryHelps {
...
export const middleware = () => () => {
  ...
  schema.pre<Doc>("save", async function (next) {
    ...
    if (this.isModified("status")) model.moveSummary(this.getChanges().$set?.status, this.status);
    next();
  });
  return schema;
};
