...
import { QuestVerifyInput, QuestVerifySchema } from "./questVerify.gql";
@Schema(dbConfig.defaultSchemaOptions)
class QuestVerify extends QuestVerifySchema {}
export const name = QuestVerify.name;
export type Input = QuestVerifyInput;
export type Raw = QuestVerify;
...
export const middleware = () => () => {
  ...
  schema.pre<Doc>("save", async function (next) {
    ...
    else if (this.status === "pending" && this.isModified("status")) model.subSummary(["total", this.status]);
